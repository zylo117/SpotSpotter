...
/**
 * This class is used to specify the parameters to use when creating a
 * thumbnail.
 * <p>
...
public class ThumbnailParameter {
	...
	/**
	 * A constant used to denote that the output format of the thumbnail should
	 * be the determined from available information such as the file name of the
	 * thumbnail.
	 * <p>
	...
	/**
	 * A constant used to denote that the default image type should be used when
	...
	/**
	 * Indicated whether or not the thumbnail should retain the aspect ratio the
	 * same as the original image when the aspect ratio of the desired
	 ...
	 * <p>
	 * A value of {@link ThumbnailParameter#DEFAULT_FORMAT_TYPE} indicates that
	 * the default type of the specified compression format should be used as
	...
	/**
	 * The {@link ResizerFactory} for obtaining a {@link Resizer} that is to be
	 ...
	 * <p>
	 * A value of {@code null} represents that the entire source image should be
	 ...
	 * @param widthScalingFactor
	 *            The scaling factor to apply to the width when creating a
	 *            thumbnail from the original image.
	 * @param heightScalingFactor
	 *            The scaling factor to apply to the height when creating a
	 *            thumbnail from the original image.
	 * @param sourceRegion
	 *            The region of the source image to use when creating a
	 *            thumbnail. A value of {@code null} indicates that the entire
	 *            source image should be used to create the thumbnail.
	 * @param keepAspectRatio
	 *            Indicates whether or not the thumbnail should maintain the
	 *            aspect ratio of the original image.
	 * @param outputFormat
	 *            A string indicating the compression format that should be
	 *            applied on the thumbnail. A value of
	 *            {@link ThumbnailParameter#ORIGINAL_FORMAT} should be provided
	 *            if the same image format as the original should be used for
	 *            the thumbnail. A value of
	 *            {@link ThumbnailParameter#DETERMINE_FORMAT} should be provided
	 *            if the output format of the thumbnail should be the determined
	 *            from the information available, such as the output file name
	 *            of the thumbnail.
	 * @param outputFormatType
	 *            A string indicating the compression type that should be used
	 *            when writing the thumbnail. A value of
	 *            {@link ThumbnailParameter#DEFAULT_FORMAT_TYPE} should be
	 *            provided if the thumbnail should be written using the default
	 *            compression type of the codec specified in
	 *            {@code outputFormat}.
	 * @param outputQuality
	 *            A value from {@code 0.0f} to {@code 1.0f} which indicates the
	 *            quality setting to use for the compression of the thumbnail.
	 *            {@code 0.0f} indicates the lowest quality, {@code 1.0f}
	 *            indicates the highest quality setting for the compression.
	 *            {@link ThumbnailParameter#DEFAULT_QUALITY} should be specified
	 *            when the codec's default compression quality settings should
	 *            be used.
	 * @param imageType
	 *            The {@link BufferedImage} image type of the thumbnail. A value
	 *            of {@link ThumbnailParameter#DEFAULT_IMAGE_TYPE} should be
	 *            specified when the default image type should be used when
	 *            creating the thumbnail.
	 * @param filters
	 *            The {@link ImageFilter}s to apply to the thumbnail. A value of
	 *            {@code null} will be recognized as no filters are to be
	 *            applied. The filters are applied after the original image has
	 *            been resized.
	 * @param resizerFactory
	 *            The {@link ResizerFactory} for obtaining a {@link Resizer}
	 *            that is to be used when performing an image resizing
	 *            operation.
	 * @param fitWithinDimensions
	 *            Whether or not to fit the thumbnail within the specified
	 *            dimensions.
	 *            <p>
	 *            If {@code true} is specified, then the thumbnail will be sized
	 *            to fit within the specified dimensions, if the thumbnail is
	 *            going to exceed those dimensions.
	 * @param useExifOrientation
	 *            Whether or not to use the Exif metadata to determine the
	 *            orientation of the thumbnail.
	 *            <p>
	 *            If {@code true} is specified, then the Exif metadata will be
	 *            used to determine the orientation of the thumbnail.
	 * 
	 * @throws IllegalArgumentException
	 *             If the scaling factor is not a rational number or is less
	 *             than or equal to 0, or if the {@link ResizerFactory} is null.
	 */
	private ThumbnailParameter(Dimension thumbnailSize, double widthScalingFactor, double heightScalingFactor,
			Region sourceRegion, boolean keepAspectRatio, String outputFormat, String outputFormatType,
			float outputQuality, int imageType, List<ImageFilter> filters, ResizerFactory resizerFactory,
		...
		/*
		 * Note: The value of DEFAULT_QUALITY is Float.NaN which cannot be
		 * compared by using the regular == operator. Therefore, to check that
		 * NaN is being used, one must use the Float.NaN method.
		 */
		if ((outputQuality < 0.0f || outputQuality > 1.0f) && !Float.isNaN(outputQuality)) {
			throw new IllegalArgumentException(
					"The output quality must be " + "between 0.0f and 1.0f, or Float.NaN to use the default "
	 ...
	 *            been resized.
	 * @param resizer
	 *            The {@link Resizer} to use when performing the resizing
	 *            operation to create a thumbnail.
	 * @param fitWithinDimensions
	 *            Whether or not to fit the thumbnail within the specified
	 *            dimensions.
	 *            <p>
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             If size is {@code null} or if the dimensions are negative, or
	 *             if the {@link Resizer} is null.
	 * @since 0.4.3
	 */
	public ThumbnailParameter(Dimension thumbnailSize, Region sourceRegion, boolean keepAspectRatio,
			String outputFormat, String outputFormatType, float outputQuality, int imageType, List<ImageFilter> filters,
			Resizer resizer, boolean fitWithinDimensions, boolean useExifOrientation) {
		this(thumbnailSize, Double.NaN, Double.NaN, sourceRegion, keepAspectRatio, outputFormat, outputFormatType,
				outputQuality, imageType, filters, new FixedResizerFactory(resizer), fitWithinDimensions,
				useExifOrientation);
	 ...
	 * @throws IllegalArgumentException
	 *             If the scaling factor is not a rational number or is less
	 *             than or equal to 0, or if the {@link Resizer} is null.
	 * @since 0.4.3
	 */
	public ThumbnailParameter(double widthScalingFactor, double heightScalingFactor, Region sourceRegion,
			boolean keepAspectRatio, String outputFormat, String outputFormatType, float outputQuality, int imageType,
			List<ImageFilter> filters, Resizer resizer, boolean fitWithinDimensions, boolean useExifOrientation) {
		this(null, widthScalingFactor, heightScalingFactor, sourceRegion, keepAspectRatio, outputFormat,
				outputFormatType, outputQuality, imageType, filters, new FixedResizerFactory(resizer),
				fitWithinDimensions, useExifOrientation);
	...
	public ThumbnailParameter(Dimension thumbnailSize, Region sourceRegion, boolean keepAspectRatio,
			String outputFormat, String outputFormatType, float outputQuality, int imageType, List<ImageFilter> filters,
			ResizerFactory resizerFactory, boolean fitWithinDimensions, boolean useExifOrientation) {
		this(thumbnailSize, Double.NaN, Double.NaN, sourceRegion, keepAspectRatio, outputFormat, outputFormatType,
				outputQuality, imageType, filters, resizerFactory, fitWithinDimensions, useExifOrientation);
	...
	public ThumbnailParameter(double widthScalingFactor, double heightScalingFactor, Region sourceRegion,
			boolean keepAspectRatio, String outputFormat, String outputFormatType, float outputQuality, int imageType,
			List<ImageFilter> filters, ResizerFactory resizerFactory, boolean fitWithinDimensions,
			boolean useExifOrientation) {
		this(null, widthScalingFactor, heightScalingFactor, sourceRegion, keepAspectRatio, outputFormat,
				outputFormatType, outputQuality, imageType, filters, resizerFactory, fitWithinDimensions,
				useExifOrientation);
	 ...
	 * 
	 * @return {@code true} if the thumbnail is to maintain the aspect ratio of
	 *         the original image, {@code false} otherwise.
	 */
	 ...
	 * <p>
	 * The value is in the range of {@code 0.0f} to {@code 1.0f}, where
	 * {@code 0.0f} is for the lowest quality setting and {@code 1.0f} for the
	 ...
	 * 
	 * @return The {@code Region} object representing the source region to use
	 *         when creating a thumbnail.
	 *         <p>
	 *         A value of {@code null} indicates that the entire source image
	 ...
	 * dimensions.
	 * 
	 * @return {@code true} is returned when the thumbnail should be sized to
	 *         fit within the specified dimensions, if the thumbnail is going to
	...
	/**
	 * Returns whether or not the Exif metadata should be used to determine the
	 * orientation of the thumbnail.
	 *
	 * @return {@code true} is returned when the Exif metadata should be used to
	 *         decide the orientation of the thumbnail, {@code false} otherwise.
	 * @since 0.4.3
	 */
