...
/**
 * An utility class used to obtain the orientation information from a given Exif
 * metadata.
 * 
 * @author coobird
 *
...
public final class ExifUtils {
	 ...
	 * 
	 * @param reader
	 *            An {@link ImageReader} which is reading the target image.
	 * @param imageIndex
	 *            The index of the image from which the Exif metadata should be
	 *            read from.
	 * @return The orientation information obtained from the Exif metadata, as a
	 *         {@link Orientation} enum.
	 * @throws IOException
	 *             When an error occurs during reading.
	 * @throws IllegalArgumentException
	 *             If the {@link ImageReader} does not have the target image
	...
	private static Orientation getOrientationFromExif(byte[] exifData) {
		...
		/*
		 * The first 2 bytes of the TIFF header contains either: "II" for Intel
		 * byte alignment (little endian), or "MM" for Motorola byte alignment
		 * (big endian)
		 */
		ByteOrder bo;
	...
	private static IfdStructure readIFD(byte[] ifd, ByteOrder bo) {
		...
		/*
		 * Per section 4.6.2 of the Exif Spec, if value is smaller than 4 bytes,
		 * it will exist in the earlier byte.
		 */
