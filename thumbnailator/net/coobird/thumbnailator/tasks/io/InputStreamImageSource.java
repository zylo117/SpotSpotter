...
/**
 * An {@link ImageSource} which uses an {@link InputStream} to read the source
 * image.
 * 
 * @author coobird
 *
...
public class InputStreamImageSource extends AbstractImageSource<InputStream> {
	 ...
	 * 
	 * @param is
	 *            The {@link InputStream} which is to be used to obtain the
	 *            source image.
	 * @throws NullPointerException
	 *             If the {@link InputStream} is {@code null}.
	 */
	...
	public BufferedImage read() throws IOException {
		 ...
		 */
		if (param != null && "true".equals(System.getProperty("thumbnailator.conserveMemoryWorkaround")) && width > 1800
				&& height > 1800 && (width * height * 4 > Runtime.getRuntime().freeMemory() / 4)) {
			int subsampling = 1;
			...
			if (param.getSize() != null && (param.getSize().width * 2 < width && param.getSize().height * 2 < height)) {
				...
				subsampling = (int) Math.max(1,
						Math.floor(1 / Math.max(param.getHeightScalingFactor(), param.getWidthScalingFactor())));
			}

			// Prevent excessive subsampling that can ruin image quality.
			// This will ensure that at least a 600 x 600 image will be used as
			// source.
			for (; (width / subsampling) < 600 || (height / subsampling) < 600; subsampling--)
				;

			// If scaling factor based resize is used, need to change the
			// scaling factor.
			if (param.getSize() == null) {
				try {
					...
					// If we can't update the parameter, then disable
					// subsampling.
					subsampling = 1;
				}
			}
		 ...
		 * `OutOfMemoryError`s which began appearing frequently with Java 7
		 * Update 21.
		 * 
