 ...
 * <p>
 * Under certain circumstances, the destination file can change in the course of
 * processing.
 * <p>
 * This can occur in cases where the file extension does not match the output
 * format set by the {@link #setOutputFormatName(String)} method. In this case,
 * the file name will have a file extension corresponding to the output format
 * set in the above method to be appended to the file name originally provided
 * when instantiating the {@link FileImageSink} object.
 * 
 * @author coobird
 *
 */
public class FileImageSink extends AbstractImageSink<File> {
	 ...
	 * <p>
	 * Under certain circumstances, the {@link File} object can be replaced in
	 * the course of processing. This can occur in cases where the file
	 * extension has been changed due to incongruence between the extension and
	 ...
	 * <p>
	 * The output format to use will be determined from the file extension. If
	 * another format should be used, then the
	 ...
	 *            Whether or not the {@code FileImageSink} should overwrite the
	 ...
	 * @throws NullPointerException
	 *             If the specified file path is {@code null}.
	 */
	...
	/**
	 * Determines whether an specified format name and file extension are for
	 * the same format.
	 * 
	 * @param formatName
	 *            Format name.
	 * @param fileExtension
	 *            File extension.
	 * @return Returns {@code true} if the specified file extension is valid for
	 ...
	 *             When an unsupported format has been specified by the
	 *             {@link #setOutputFormatName(String)} method, or if the output
	 *             format has not been set and cannot be determined from the
	 *             file name.
	 * @throws IOException
	 *             When a problem occurs while writing the image.
	 * @throws NullPointerException
	 *             If the image is {@code null}.
	 * @throws IllegalArgumentException
	 *             If this {@code FileImageSink} does not permit overwriting the
	 *             destination file and the destination file already exists.
	 */
	public void write(BufferedImage img) throws IOException {
		super.write(img);

		/*
		 * TODO refactor. The following code has been adapted from the
		 ...
		 * 
		 * If the file extension matches the output format's extension, then
		 * leave as is.
		 * 
		...
		if (writeParam.canWriteCompressed() && param != null) {
			 ...
			 * 
			 * Note: The value to denote that the codec's default compression
			 * type should be used is null.
			 */
			 ...
			 * 
			 * Note: The value to denote that the codec's default compression
			 * quality should be used is Float.NaN.
			 */
		...
		/*
		 * Here, an explicit FileOutputStream is being created, as using a File
		 * object directly to obtain an ImageOutputStream was causing a problem
		 * where if the destination file already exists, then the image data was
		 * being written to the beginning of the file rather than creating a new
		 * file.
		 */
		...
		/*
		 * The following two lines used to be surrounded by a try-catch, but it
		 * has been removed, as the IOException which it was throwing in the
		 * catch block was not giving good feedback as to what was causing the
		 * original IOException.
		 * 
		 * It would have been informative to have the IOException which caused
		 * this problem, but the IOException in Java 5 does not have a "cause"
		 * parameter.
		 * 
		 * The "cause" parameter has been introduced in Java 6:
		 * http://docs.oracle.com/javase/6/docs/api/java/io/IOException.html#
		 * IOException%28java.lang.String,%20java.lang.Throwable%29
		 *
		 * TODO Whether to surround this portion of code in a try-catch again is
		 * debatable, as it wouldn't really add more utility.
		 *
		 * Furthermore, there are other calls in this method which will throw
		 * IOExceptions, but they are not surrounded by try-catch blocks. (A
		 * similar example exists in the OutputStreamImageSink where the
		 * ImageIO.createImageOutputStream is not surrounded in a try-catch.)
		 *
		 * Related issue:
		...
		/*
		 * Note: The following code is a workaround for the JPEG writer which
		 * ships with the JDK.
		 * 
		 * At issue is, that the JPEG writer appears to write the alpha channel
		 * when it should not. To circumvent this, images which are to be saved
		 * as a JPEG will be copied to another BufferedImage without an alpha
		 * channel before it is saved.
		 * 
