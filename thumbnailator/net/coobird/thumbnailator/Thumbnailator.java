...
public final class Thumbnailator {
	 ...
	 * 
	 * @param task
	 *            A {@link ThumbnailTask} to execute.
	 * @throws IOException
	 *             Thrown when a problem occurs when creating a thumbnail.
	 */
	public static void createThumbnail(ThumbnailTask<?, ?> task) throws IOException {
		...
		if (param.getSize() != null) {
			...
			destinationImage = new FixedSizeThumbnailMaker().size(destinationWidth, destinationHeight)
					.keepAspectRatio(param.isKeepAspectRatio()).fitWithinDimensions(param.fitWithinDimenions())
					.imageType(imageType).resizerFactory(param.getResizerFactory()).make(sourceImage);
		} else if (!Double.isNaN(param.getWidthScalingFactor())) {
			// Create the thumbnail.
			destinationImage = new ScaledThumbnailMaker()
					.scale(param.getWidthScalingFactor(), param.getHeightScalingFactor()).imageType(imageType)
					.resizerFactory(param.getResizerFactory()).make(sourceImage);
		} else {
			throw new IllegalStateException(
					"Parameters to make thumbnail" + " does not have scaling factor nor thumbnail size specified.");
		}
	 ...
	 * thumbnail will be 50 pixels by 50 pixels, as the constraint will be 50
	 * pixels for the width, and therefore, by preserving the aspect ratio, the
	...
	public static BufferedImage createThumbnail(BufferedImage img, int width, int height) {
		...
		BufferedImage thumbnailImage = new FixedSizeThumbnailMaker(width, height, true, true).resizer(resizer)
				.make(img);
	...
	/**
	 * Creates a thumbnail from an source image and writes the thumbnail to a
	 * destination file.
	 * <p>
	 ...
	 * the same image format as the original image will be used when writing the
	 * thumbnail.
	 * 
	 * @param inFile
	 *            The {@link File} from which image data is read.
	 * @param outFile
	 *            The {@link File} to which thumbnail is written.
	 * @param width
	 *            The width of the thumbnail.
	 * @param height
	 *            The height of the thumbnail.
	 * @throws IOException
	 *             Thrown when a problem occurs when reading from {@code File}
	 ...
	 * 
	 * @param is
	 *            The {@link InputStream} from which to obtain image data.
	 * @param os
	 *            The {@link OutputStream} to send thumbnail data to.
	 * @param width
	 *            The width of the thumbnail.
	 * @param height
	 *            The height of the thumbnail.
	 ...
	 *            The {@link OutputStream} to send thumbnail data to.
	 * @param format
	 *            The image format to use to store the thumbnail data.
	 * @param width
	 *            The width of the thumbnail.
	 * @param height
	 *            The height of the thumbnail.
	 ...
	 * The filenames of the resulting thumbnails are determined by applying the
	 * specified {@link Rename}.
	 * <p>
	 * The order of the thumbnail {@code File}s in the returned
	 * {@code Collection} will be the same as the order as the source list.
	 * 
	 * @param files
	 *            A {@code Collection} containing {@code File} objects of image
	 *            files.
	 * @param rename
	 *            The renaming function to use.
	 * @param width
	 *            The width of the thumbnail.
	 * @param height
	 *            The height of the thumbnail.
	 ...
	 *             {@link Thumbnails#fromFiles(Iterable)} interface. This method
	 *             will be removed in 0.5.0, and will not be further maintained.
	 */
	public static Collection<File> createThumbnailsAsCollection(Collection<? extends File> files, Rename rename,
	 ...
	 * The filenames of the resulting thumbnails are determined by applying the
	 * specified {@code Rename} function.
	 * 
	 * @param files
	 *            A {@code Collection} containing {@code File} objects of image
	 *            files.
