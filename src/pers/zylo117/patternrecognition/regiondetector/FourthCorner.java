package pers.zylo117.patternrecognition.regiondetector;
...
public class FourthCorner {
	...
	public static Point fourthPoint(Point p1, Point p2, Point p3) {
		Point p4;
//		double diagonal;
//		diagonal = Math.sqrt(Math.pow((p3.x - p1.x), 2) + Math.pow((p3.y - p1.y), 2) + Math.pow((p2.x - p1.x), 2)
...
//		double b = 2 * y21 * y31 * x21 - 2 * Math.pow(y21, 2) * p3.x - 2 * Math.pow(x21, 2) * p1.x;
//		double c = -Math.pow(x21, 2) * Math.pow(diagonal, 2) + Math.pow(x21, 2) * Math.pow(p1.x, 2)
