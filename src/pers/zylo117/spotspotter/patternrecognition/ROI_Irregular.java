...
public class ROI_Irregular {
	...
	public static Mat RectangleSubROI(Mat input, Rect rect) {
		...
		Imgproc.floodFill(maskCopyTo, maskFloodFill,
				...
				Scalar.all(20), Scalar.all(20), 4); // 漫水填充法填充圆的内部
	...
	public static Mat Circle(Mat circularInput, Point cc, int radius) {
		...
		final Mat maskFloodFill = Mat.zeros(new Size(circularInput.cols() + 2, circularInput.rows() + 2),
				CvType.CV_8UC1); // 创建floodFill方法的mask，尺寸比原图大一些
	...
	// public static Mat zoomoutMat(Mat input, Point p1, Point p2, Point p3, Point
	// p4, double scale_x, double scale_y) {
	...
	//
	// Mat out = irregularQuadrangle_Simplified(input, newP1, newP2, newP3, newP4,
	// shift, noULandLRcorner, ulCornerRatio, lrCornerRatio)
	// return null;
	// }
}
