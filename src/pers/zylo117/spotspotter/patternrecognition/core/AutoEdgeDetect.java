...
public class AutoEdgeDetect {
	...
	public static Mat autoCanny(Mat image, double sigma, boolean ifBlur, double inwardCropRatio) {
		...
		final Rect rect = new Rect((int) (image.width() * inwardCropRatio / 2), (int) (image.height() * inwardCropRatio / 2),
				(int) (image.width() * (1 - inwardCropRatio)), (int) (image.height() * (1 - inwardCropRatio)));

		final Mat sub = new Mat(image, rect);

		final byte[] array = GetPixelArray.getRaster(sub);
		// for (int i = 0; i < image.width() * image.height(); i++) {
		// if (i == 25)
		// System.out.println(array[i]);
		// }

		final List<Integer> intRaseter = new ArrayList<>();
		...
		final int length = intRaseter.size();
		if (length % 2 == 0)
			median = (intRaseter.get(intRaseter.size() / 2 - 1) + intRaseter.get(intRaseter.size() / 2)) / 2;
		else
			median = intRaseter.get(intRaseter.size() / 2);

		final int lower = (int) (0 < ((1.0 - sigma) * median) ? ((1.0 - sigma) * median) : 0);
		final int upper = (int) (255 > ((1.0 + sigma) * median) ? ((1.0 + sigma) * median) : 255);

		final Mat blurred = new Mat();
		final Mat edge = Mat.zeros(image.size(), CvType.CV_8UC1);
		if (ifBlur) {
	...
	public static Mat[] largestContour(Mat raw, Mat crop, double minArea, int offset) {
		// 0为ROI，1为标示框
		final Mat[] matSet = new Mat[3];

		final List<MatOfPoint> cnts = new ArrayList<MatOfPoint>();
		...
		final List<Map.Entry<Integer, Double>> cnts_AreaOrder = new ArrayList<Map.Entry<Integer, Double>>(map.entrySet());
		...
		// System.out.println(cnts_AreaOrder.get(i).getKey() + ":" +
		// cnts_AreaOrder.get(i).getValue());
		// }
		...
		// // 画出最大的区域
		final Mat roi = Mat.zeros(crop.size(), CvType.CV_8UC3);
		...
		// roi = ROI_Irregular.irregularQuadrangle_Simplified(raw, vertices[1],
		// vertices[2], vertices[0], vertices[3], 0, 0, false, 0, 0);
		// MatView.imshow(roi, "Rect");

		Imgproc.drawContours(raw, cnts, minIdx, new Scalar(200, 128, 128), offset * 2);

		final Mat zero = Mat.zeros(crop.size(), CvType.CV_8UC3);
